...
export function DishCard({
  ...
  const isLoggedIn = isClerkSignedIn || isSessionLoggedIn;
        ...
        <div>
          <div className="flex items-start justify-between gap-2">
            ...
            <div className="flex items-center gap-1.5 shrink-0">
              <div className="px-2 py-0.5 bg-amber-500/10 border border-amber-500/30 text-amber-400 rounded-lg flex items-center gap-1 text-[11px] font-black">
                <Star className="w-3 h-3 fill-amber-400 text-amber-400" />
                <span>{dish.avgRating ?? 4.8}</span>
                <span className="text-slate-400 text-[10px] font-semibold">({dish.reviewCount ?? 12})</span>
              </div>

              <button
                onClick={() => setShowRatingModal(true)}
                className="px-2.5 py-1 bg-amber-500/20 hover:bg-amber-500/30 border border-amber-500/40 text-amber-400 rounded-xl flex items-center gap-1 text-[11px] font-extrabold transition-all active:scale-95 shadow-sm"
